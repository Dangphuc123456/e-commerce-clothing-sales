...
const ProductPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                const res = await api.get("/api/customer/products-all");
    ...
    return (
        <Container className="my-4">
            ...
            <div className="d-flex flex-nowrap">
                ...
                <div style={{ flex: "0 1 70%", minWidth: "300px", marginRight: "15px" }}>
                    <Row xs={1} sm={2} md={3} className="g-4">
                        {getCurrentPageProducts().map((prod) => (
                            <Col key={prod.id}>
                                <Card className="h-100 shadow-sm product-card" onClick={() => navigate(`/product/${prod.slug}`)}>
                                    ...
                                    <Card.Body>
                                        ...
                                        <div className="d-flex justify-content-center align-items-center mt-2 gap-2">
                                            <span className="fw-bold text-danger">{formatCurrency(prod.discounted_price || prod.price)}</span>
                                            {prod.discounted_price && prod.discounted_price < prod.price && (
                                                <small className="text-muted text-decoration-line-through">{formatCurrency(prod.price)}</small>
                                            )}
                                        </div>
                                    </Card.Body>
                                </Card>
                ...
                {/* Cột sản phẩm mới & bán chạy */}
                <div style={{ flex: "0 1 30%", minWidth: "200px" }}>
                    <h5 className="mb-3 fs-3">Sản phẩm mới</h5>
                    {latestProducts.map((prod) => (
                        <Card key={prod.id} className="mb-3 shadow-sm p-2" style={{ cursor: "pointer", width: "100%" }} onClick={() => navigate(`/product/${prod.slug}`)}>
                            <div className="d-flex align-items-center flex-nowrap">
                                {prod.image && (
                                    <img
                                        src={prod.image}
                                        alt={prod.name}
                                        style={{ width: "60px", height: "90px", objectFit: "cover", borderRadius: "4px", marginRight: "10px", flexShrink: 0 }} />
                                )}
                                <div className="flex-grow-1 text-truncate">
                                    <Card.Title className="mb-1" style={{ fontSize: "0.9rem", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                                        {prod.name}
                                    </Card.Title>
                                    <div>
                                        <span className="fw-bold text-danger" style={{ fontSize: "0.85rem" }}>{formatCurrency(prod.discounted_price || prod.price)}</span>
                                        {prod.discounted_price && prod.discounted_price < prod.price && (
                                            <small className="text-muted text-decoration-line-through ms-1" style={{ fontSize: "0.75rem" }}>{formatCurrency(prod.price)}</small>
                                        )}
                                    </div>
                                </div>
                            </div>
